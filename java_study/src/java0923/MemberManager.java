...
public class MemberManager extends JFrame {
	...
	public MemberManager() {
		...
		btnAdd.addActionListener((event) -> {
			...
			String id = tfId.getText().trim();// 좌우 공백제거 trim()
			...
			MemberVo memberVo = new MemberVo(id, passwd, name);// MemberVo랑 연결
			memberDao.addMember(memberVo);// 전달
...
//			String name = tfName.getText().trim();
			...
			// 아이디값 없으면 "삭제할 아이디를 입력하세요" 다이얼로그 띄움
			if (id.length() == 0) {
			...
			// 아이디값 있으면 "정말 삭제하시겠습니까?" 물어보기
			int result = JOptionPane.showConfirmDialog(MemberManager.this, "정말 삭제 하시겠습니까?", "Confirm",
